...
usage() {
  cat <<'EOF'
Usage: generate-github-env.sh --sa-key-file PATH [--project-id ID] [--tfstate-bucket NAME] [--output FILE]

Writes a shell-style env file with the GCP values needed for GitHub.
EOF
}
...
if [[ -z "$output_file" ]]; then
  output_file="_config/github-gcp.env"
fi
