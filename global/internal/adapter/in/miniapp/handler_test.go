...
func TestStaticMiniAppIsEmbeddedWithSecurityHeaders(t *testing.T) {
	...
	if !strings.Contains(string(script), "requestPermission") ||
		!strings.Contains(string(script), "webkitCompassHeading") ||
	...
	if !strings.Contains(string(script), "async function updateLocation(button) {\n    if (!isTelegramMobile()) {\n      showFeatureNotice(state.labels.home_help)") {
		t.Fatal("Mini App location sharing must explain its mobile requirement before requesting location")
	}
	...
	if strings.Contains(html, `id="occasions-title"`) || strings.Contains(html, "reminder-occasions-title") ||
		strings.Contains(html, "reminder-group-heading") || strings.Contains(string(script), "occasion-category") ||
...
func TestLookupReturnsScheduleWithoutChangingProfile(t *testing.T) {
	...
	resolver := &fakeResolver{resolved: domain.ResolvedLocation{Timezone: "Africa/Cairo", City: "Cairo", CountryCode: "EG"}}
	planner := &fakePlanner{}
...
func TestLocationUpdatePersistsCountryCode(t *testing.T) {
	...
	storage := newFakeStorage()
	storage.chats[42] = domain.Chat{TelegramChatID: 42, Type: "private", LanguageCode: "en"}
	resolver := &fakeResolver{resolved: domain.ResolvedLocation{
...
func (s *fakeStorage) Profile(_ context.Context, chatID int64) (domain.PrayerProfile, error) {
	...
	if !ok {
		return domain.PrayerProfile{}, domain.ErrNotFound
	}
	return profile, nil
}
...
func (s *fakeStorage) MetalPrices(_ context.Context) (domain.MetalPrices, error) {
	if s.metalPrices == nil {
		return domain.MetalPrices{}, domain.ErrNotFound
	}
...
func (r *fakeResolver) Resolve(_ context.Context, latitude, longitude float64) (domain.ResolvedLocation, error) {
