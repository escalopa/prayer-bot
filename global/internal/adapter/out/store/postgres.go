...
// notFound translates pgx's absence sentinel into the domain error at the
// adapter boundary, so no consumer imports pgx to classify errors.
...
func (s *Store) AdminMetrics(ctx context.Context) (domain.AdminDashboard, error) {
...
func (s *Store) metricCounts(ctx context.Context, query string) ([]domain.MetricCount, error) {
	rows, err := s.pool.Query(ctx, query)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var metrics []domain.MetricCount
	for rows.Next() {
		var metric domain.MetricCount
...
func (s *Store) Profile(ctx context.Context, chatID int64) (domain.PrayerProfile, error) {
	...
	if err != nil {
		return domain.PrayerProfile{}, notFound(err)
	}
...
func (s *Store) PendingOutbox(ctx context.Context, limit int) ([]domain.OutboxItem, error) {
...
func (s *Store) MetalPrices(ctx context.Context) (domain.MetalPrices, error) {
	...
	if err != nil {
		return domain.MetalPrices{}, notFound(err)
	}
